...
source("/data/share/htp/pleiotropy/paper_data/DA_ipsc_npc/scripts/TFBS_analysis_functions_ATAC.R")
...
get_TFBS_positions_ATAC<-function(get_region_id, human_file, macaque_file,
  ...
  genomic_pos<-readRDS("DA_ipsc_npc/cbust/RDS/jamm_inATAC.rds") %>%
...
plot_TFBScoverage_ATAC<-function(outfile = outfile, get_region_id, speciesColors, padding = 0, 
                                 ...
                                 tmpdir="DA_ipsc_npc/cbust/RDS/TFBS_coverage/tmp/"){
...
plot_ATAC<-function(genome, bw_sample, get_region_id, fill_col, padding = 0, 
  ...
  # coordinate files
  if (genome == "hg38"){
    coord = setNames(read.table("DA_ipsc_npc/bed/hg38_DHS.bed"), c("seqnames", "start", "end", "region_id"))
  }
  if (genome == "macFas6"){
    coord = setNames(read.table("DA_ipsc_npc/bed/macFas6_DHS.bed"), c("seqnames", "start", "end", "region_id"))
  }
...
get_chip<-function(name, jamm=jamm_inATAC, dnase=DHS_genes, exprtf=expressedTFs,
  ...
  if (old == TRUE){
    chip<-read.table(paste0("DA_ipsc_npc/chipseq_gtrd/macs2 peaks_Homo sapiens.interval_target",name,"_allTFs")) 
  } else {
    chip<-read.table(paste0("DA_ipsc_npc/chipseq_gtrd/macs2 peaks_Homo sapiens.interval_",name)) 
  }
