...
combis<-list.files("DA_ipsc_npc/cbust/RDS/TFBS_position/combis_exprTissues_10perc/", full.names = TRUE)
...
TFBS_dist<-lapply(combis, function(x){
  ...
  if (dim(combi$TF_out)[1]>0){
    ...
    return(data.frame(region_id=gsub("DA_ipsc_npc/cbust/RDS/TFBS_position/combis_exprTissues_10perc/region_id_","", x)))
  }
})

TFBS_dist<-lapply(TFBS_dist, function(x){x %>% dplyr::mutate(region_id = as.factor(as.character(region_id)))}) %>% bind_rows()

saveRDS(TF_dist, "DA_ipsc_npc/cbust/RDS/TFBS_position/summarized_TFBS_distances_exprTissues_10perc.rds")
