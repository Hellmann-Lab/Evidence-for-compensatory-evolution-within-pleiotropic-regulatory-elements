...
jamm_region_identifiers<-readRDS("../general/region_summary/jamm_region_identifiers.rds")
jamm_DNase_binary<-readRDS("../general/region_summary/jamm_DNase_binary.rds")
jamm_region_info_full<-readRDS("../general/region_summary/jamm_region_info_full.rds")
jamm_peaks<-readRDS("../general/region_summary/jamm_peaks.rds")
...
for (i in 1:9){
  ...
  write.table(paste0("../general/bed/full_sp_bed/sp",i,".bed"),sep="\t",row.names = F,col.names = F,quote=F)
}
...
for (i in 1:2){
bed<-read.table(paste0("../general/bed/full_sp_bed/sp",i,".bed"),head=F)
tmp<-data.frame(chr=bed[,1],sta=bed[,2],end=bed[,3])

ll=dim(tmp)[1]
outfile<-paste0("../general/bed/full_sp_bed/sp",i,".downs.bed")
if( sum(tmp$end-tmp$sta)/2e7>1){
  ...
  for (p in 1:parts){
    write.table(tmp[(vec[p]+1):(vec[p+1]),],sep="\t",row.names = F,col.names = F,quote=F,file=paste0("../general/bed/full_sp_bed/split/sp",i,"_part",p,".bed"))
  }
} else { 
...
DHS_annot<-readRDS("../distance_to_tss/DHS_to_gene.rds") %>%
...
for (i in c(1:9)){
  ...
  bed_file<-read.table(paste0("../general/bed/full_sp_bed/sp",i,".bed"))
...
target_dir="../general/bed/PeaksAntiPeaks"
...
for (i in c(2:9)){
  ...
  bed_file<-read.table(paste0("../general/bed/PeaksAntiPeaks/sp",i,"_",t,"_",p,".bed"))
...
stringent_set_INSIGHT<-readRDS("../DA_ipsc_npc/RDS/jammPeaks_vs_ATACPeaks_NPC_stringent.rds") %>%
...
cov<-read.table("../general/bed/peak_coverage.bed")
...
for (i in 1:9){
  ...
  if( sum(tmp$end-tmp$sta)/2e7>1){
    ...
    print(n)
    #downsample
    write.table(tmp[sample.int(n=dim(tmp)[1], size=n), ],sep="\t",row.names = F,col.names = F,quote=F,file=paste0("../general/bed/maxTissuePeak_bed/sp",i,".downs.bed"))
  } else {
    write.table(tmp, file=paste0("../general/bed/maxTissuePeak_bed/sp",i,".bed"),sep="\t",row.names = F,col.names = F,quote=F)
  }}
...
for (i in c("1.downs", "2.downs",3:9)){
  ...
  bed_file<-read.table(paste0("../general/bed/maxTissuePeak_bed/sp",i,".bed"))
  colnames(bed_file)<-c("seqnames","start","end")
