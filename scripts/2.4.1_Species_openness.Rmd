...
saveRDS(comb.lfc, "../expression_conservation/RDS/DE_DA_table.rds")
...
lfc_npc_summary<-LFC_sum %>% mutate(PD = as.numeric(PD),
  ...
  basic_theme_ins2+
  theme(legend.position = c(0.8,0.83), legend.title=element_blank())
...
enrichTest2<- comb.lfc %>% 
  filter(region_id %in% jamm_forATAC_NPC_all$region_id[jamm_forATAC_NPC_all$openness!="Not open"]) %>%
  group_by( total, celltype) %>% 
  summarise(DE_DA = sum(padj<=0.1 & padj.da<=0.1,na.rm=T),
            DE_notDA = sum(padj<=0.1 & padj.da>0.1,na.rm=T),
            notDE_DA = sum(padj>0.1 & padj.da<=0.1,na.rm=T),
            notDE_notDA = sum(padj>0.1 & padj.da>0.1,na.rm=T)) %>% 
  pivot_longer(DE_DA:notDE_notDA, names_to = "cnt_type", values_to = "cnt" ) %>% 
  group_by( total, celltype) %>% 
...
enrichTest2$padj <- p.adjust(enrichTest2$p.value, method = "BH")

enrichTest2_NPC_plot_together<-enrichTest2  %>% 
  filter(celltype == "NPC") %>% 
  ...
  geom_pointrange(size=0.3, color="grey30")+ 
  geom_text(aes(y=estimate+0.2),
...
fig3_top<-plot_grid(fig2A+theme(axis.text = element_text(color="black")),
                    ...
                    scale=c(0.9,1, 1), labels=c("A","","B"), label_size=12, label_x = c(0,-0.05))

fig3_middle<-plot_grid(mamtree,evoage, rel_widths = c(1,1.3), scale = c(0.8, 1), labels=c("C","D"), label_size=12, label_x = c(0,-0.05))

fig3_bottom<-plot_grid(lfc_npc_summary+theme(legend.position=c(0.75,0.82)),
...
ggsave(file="../figures/new_figure3.pdf",width =162.5*0.75, height=160, units = "mm")
...
jamm_DNase_binary<-readRDS("../roadmap_DHS_summaries/region_summary/jamm_region_info_full.rds") %>%
  ...
  right_join(jamm_region_info_full %>% dplyr::select(region_id))
...
plot_grid(
  ...
  dendro, 
  #NULL,
  plot_grid(NULL,enrichTest2_NPC_plot_together +theme(axis.text=element_text(color="black")),
            NULL, scale = 0.95,
            #labels = c("C","",""), label_size = 12, 
            rel_widths=c(0.25,1,0.25), ncol = 3),
  ncol=1, rel_heights=c(1.1,0.6,0.6), labels = c("","C","D"), label_size =12)

ggsave("../figures/SuplFig2_combined_withDendro_new.pdf", height = 7, width = 5)
