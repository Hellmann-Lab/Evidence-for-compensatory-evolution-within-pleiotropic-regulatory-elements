...
n_tissues_expressedTFs<-readRDS("../expression/summarized_expression_allTissues.rds") %>%
...
#dd_exprTissues <-  data.table::fread("../DA_ipsc_npc/cbust/topNmotifs_exprTissues_unfiltered_dt.csv")
# top 10% ####
#dd_10perc_exprTissues_filt<- dd_exprTissues[ minminRNK<=ceiling(nMotif*.1) ][ ,minminRNK:=NULL ][,nMotif:=NULL]
#data.table::fwrite(dd_10perc_exprTissues_filt, "../DA_ipsc_npc/cbust/top10perc_motifs_exprTissues_dt.csv")
...
tissues_list<-lapply(unique(expressedInTissue$tissue), function(tis){
  ...
  sel_cres<-readRDS("../distance_to_tss/DHS_to_gene.rds") %>% filter(tissue == tis)
...
library(readxl)
stripes<-read_excel("../DA_ipsc_npc/stripeTF/1-s2.0-S1097276522006128-mmc2.xlsx", sheet = 5,
...
saveRDS(combined_ranks_all2, "../DA_ipsc_npc/cbust/RDS/motif_PD_ranks.rds")
