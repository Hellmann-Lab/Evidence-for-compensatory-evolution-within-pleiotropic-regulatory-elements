...
ggplot(div_df, aes(x = log(diversityH)))+geom_histogram()
ggplot(div_df, aes(x = diversityH))+geom_histogram()


#car::vif(mod_div)
true_div<-fit_permutation_diversity(div_df, shuffle=F)
shuffled_div<-lapply(1:30, function(x){
  fit_permutation_diversity(div_df, shuffle=T)
  })

shuffled_div_df<-bind_rows(shuffled_div, .id = "iteration") %>% 
  group_by(Predictor) %>% 
  dplyr::summarise(R2 = mean(R2),
                   lowerCI_R2 = quantile(R2, probs= 0.01),
                   upperCI_R2 = quantile(R2, probs = 0.99))


combined_r2<-bind_rows(true_div %>% dplyr::mutate(type="Observed"),
                       shuffled_div_df %>% dplyr::mutate(type="Shuffled"))

preddiv<-combined_r2 %>% 
  filter(Predictor!="(Intercept)") %>% 
  dplyr::mutate(Predictor=gsub("Promoter","",Predictor),
                Predictor=gsub("_"," ", Predictor),
                Predictor=gsub("total", "PD", Predictor),
                type = ifelse(type=="Observed","Actual","Shuffled PD")) %>% 
  ggplot(aes(x=Predictor, y = R2, fill=type)) +
  geom_point(size=1.8,colour="black",pch=21, position=position_dodge(width=0.44)) +
  geom_errorbar(aes(ymin = lowerCI_R2, ymax = upperCI_R2), width=0.6, linewidth=0.3,
                position = position_dodge(width=0.44))+
  scale_fill_manual(values = c("Actual" = "#A28CA6",
                                "Shuffled PD" = "white"))+
  ...
  labs(fill="Diversity\nprediction")+
  xlab("")+
  theme(legend.position = c(0.71,0.2))
