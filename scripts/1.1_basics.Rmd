...
saveRDS(jamm_region_identifiers, "../general/region_summary/jamm_region_identifiers.rds")

jamm_DNase_binary<-dbGetQuery(con, "SELECT * FROM jamm_DNase_binary")
saveRDS(jamm_DNase_binary, "../general/region_summary/jamm_DNase_binary.rds")
...
write.table(jamm_region_info_full[,1:3], "../general/bed/all_DHS.bed", col.names = F, row.names = F, quote = F, sep = "\t")

saveRDS(jamm_region_info_full, "../general/region_summary/jamm_region_info_full.rds")
...
saveRDS(dnase_jamm_long, "general/region_summary/jamm_per_tissue.rds")
...
saveRDS(jamm_peaks, "../general/region_summary/jamm_peaks.rds")

write.table(jamm_peaks[,c(1:3,9,10)], "../general/bed/all_peaks_ids.bed", col.names = F, row.names = F, quote = F, sep = "\t")
write.table(jamm_region_info_full[,c(1:4,25)], "../general/bed/all_DHS_ids.bed", col.names = F, row.names = F, quote = F, sep = "\t")

system("sbatch --wrap='bedtools coverage -d -a ../general/bed/all_DHS_ids.bed -b ../general/bed/all_peaks_ids.bed > ../general/bed/peak_coverage.bed'")
...
suppl1<-plot_grid(suppl_top,enh_expr_suppl, labels = c("","C"), ncol=1, label_size = 12, rel_heights = c(0.5,0.9), scale=c(1,0.95))

ggsave("../figures/suppl1.pdf", suppl1, height=150*0.85, width =162.5*0.75, units = "mm")
